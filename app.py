            ...
            if "image" in doc and "url" in doc["image"]:
                ...
                if response.status_code == 200:
                    ...
                    if encodings:
                        ...
                        print(f"Successfully processed image for {title}")
                    else:
                        print(f"No face found in image {title}. Skipping.")
                else:
                    print(f"Failed to download image for {title}. HTTP status code: {response.status_code}")
            else:
...
def process_frame(frame, model, class_names):
    ...
    try:
        ...
        if pose_results.pose_landmarks:
            ...
            if hip_height - shoulder_height < 0.1:  # Small vertical difference -> lying down
                cv2.putText(frame, "Fall Detected!", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
                play_alert_sound()
                log_to_csv('behavior_log.csv', ["Fall detected", current_time.strftime('%Y-%m-%d %H:%M:%S')])
